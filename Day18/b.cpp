...
std::vector<std::vector<int>> readInput(std::ifstream& is)
{
    std::vector<std::vector<int>> retNumbers;
    // Read the first line directly into the sequence.
    std::string line;

    while ( std::getline(is, line) )
    {
        retNumbers.push_back(stringToVecInt(line));
    }

    return retNumbers; 
}

int64_t solve(const std::vector<std::vector<int>> snailfishNumbers)
{
    int64_t largestSum = 0;
    for ( uint32_t i = 0; i < snailfishNumbers.size(); i++ )
    {
        for ( uint32_t j = 0; j < snailfishNumbers.size(); j++ )
        {
            if ( i == j )
            {
                continue;
            ...
            std::vector<int> number = snailfishNumbers.at(i);
            addSnailfishNumbers(number, snailfishNumbers.at(j));

            reduce(number);
            int64_t sum = computeMagnitude(number);
            if ( sum > largestSum )
                largestSum = sum;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    // Reads all snailfish numbers.
    std::vector<std::vector<int>> snailfishNumbers = readInput(is);

    int64_t largestSum = solve(snailfishNumbers);

    std::cout << "Largest sum of any two snailfish numbers is " << largestSum << std::endl;
}
