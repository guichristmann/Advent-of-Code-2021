    ...
    for ( uint32_t y = 0; y < data.size(); y++ )
    {
        for ( uint32_t x = 0; x < data.at(0).size(); x++ )
        {
            if ( data.at(y).at(x) == DATA_ON )
                ...
                std::cout << ".";
            else if ( data.at(y).at(x) == DATA_UNUSED )
                std::cout << "_";
        }
        std::cout << std::endl;
    }
...
bool isDataOn(const int x, const int y, const Image& img, Image& outResultImg)
{
    ...
    else if ( img.data.at(y).at(x) == DATA_UNUSED )
    {
        outResultImg.data.at(y).at(x) = DATA_OFF;
    }
...
void operate(const int x, const int y, const Image& img, Image& outResultImg, const Algorithm& algo)
{
    ...
    if ( isDataOn(x - 1, y - 1, img, outResultImg ) )
        binaryIdx.set(8);
    if ( isDataOn(x, y - 1, img, outResultImg ) )
        binaryIdx.set(7);
    if ( isDataOn(x + 1, y - 1, img, outResultImg ) )
        binaryIdx.set(6);

    if ( isDataOn(x - 1, y, img, outResultImg ) )
        binaryIdx.set(5);
    if ( isDataOn(x, y, img, outResultImg ) )
        binaryIdx.set(4);
    if ( isDataOn(x + 1, y, img, outResultImg ) )
        binaryIdx.set(3);
...
void enhance(Image& img, const Algorithm& algo)
{
    ...
    for ( uint32_t k_y = 1; k_y < img.data.size() - 1; k_y++ )
    {
        for ( uint32_t k_x = 1; k_x < img.data.at(0).size() - 1; k_x++ )
        {
...
{
    uint32_t count = 0;
    for ( uint32_t row = 2; row < img.data.size() - 2; row++ )
    {
        for ( uint32_t col = 2; col < img.data.at(0).size() - 2; col++ )
        {
