...
bool findOverlapAndTransform(const Scan& s1, Scan& s2)
{
...
int main(int argc, char* argv[])
{
    ...
    while ( transformQueue.size() > 0 )
    {
        ...
        {
            flag = findOverlapAndTransform(scanners.at(i), scanners.at(idxToBeTransformed));
            if ( flag )
            {
    ...
    for ( const Scan& s : scanners )
    {
        for ( const Point& p : s )
        {
            auto idx = std::find(uniqueBeacons.begin(), uniqueBeacons.end(), p);
            if ( idx == uniqueBeacons.end() )
                uniqueBeacons.push_back(p);
        }
    }

    printScan(uniqueBeacons);
    std::cout << "Total of " << uniqueBeacons.size() << " unique beacons." << std::endl;
